...
# ==========================================
# 1. CUSTOM EXCEPTIONS & OOP MODELS
# ==========================================
...
class Person:
    def __init__(self, data):
        ...
        self.phone = data.get('Phone') or data.get('Contact')
...
class Nexus:
    def __init__(self, root):
        ...
        # --- LIGHT THEME COLORS ---
        self.bg_light = "#F1F5F9"  # Main Window
        self.bg_card = "#FFFFFF"  # Containers
        self.sidebar_bg = "#FFFFFF"  # Sidebar
        self.accent_blue = "#0EA5E9"  # Primary Buttons/Links
        self.text_main = "#1E293B"  # Dark Gray text
        self.text_muted = "#64748B"  # Lighter text
    ...
    def load_data(self):
        try:
            if os.path.exists("hospital_records.json"):
                ...
                self.pay_engine.gross_revenue = content.get("revenue", 250000.0)
    ...
    def setup_main_window(self):
        self.root.title("Nexus ERP - Light Interactive Edition")
    ...
    def show_login_screen(self):
        ...
        tk.Label(self.login_frame, text="NEXUS HEALTH ERP", fg=self.accent_blue, bg=self.bg_light,
    ...
    def setup_dashboard(self):
        ...
        nav = [
            ("➕ Admit Patient", self.show_admit), ("👨‍⚕️ Register Doctor", self.show_doc_reg),
            ("👩‍⚕️ Register Nurse", self.show_nurse_reg), ("🤝 Assign Doctor", self.show_assign),
        ...
        for t, c in nav:
            ...
            if bg_c == self.sidebar_bg:  # Add hover effect to normal menu items
    ...
    def save_p(self):
        ...
        if not data["Name"]: return messagebox.showwarning("Error", "Name is required")
    ...
    def save_staff(self, role):
        ...
        data["Salary"] = manual if manual and manual.strip() != "" else self.pay_engine.calculate_auto_salary(role,
                                                                                                              data.get(
                                                                                                                  "Experience Level") or data.get(
                                                                                                                  "Qualification"))
    ...
    def show_display(self):
        ...
        for p in self.patients: tree.insert("", "end", values=(p.id, p.name, p.ward, p.status, p.assigned_doctor))
        ...
        for c in scols: stree.heading(c, text=c); stree.column(c, width=120)
        stree.pack(fill="both", expand=True)
        for s in self.staff_list: stree.insert("", "end", values=(s.name, s.role, s.shift, f"Rs.{s.salary:,.0f}",
                                                                  "✓" if s.is_paid else "✗"))
        ...
        for p in self.patients:
            if p.name == self.b_p.get():
                try:
                    ...
                    res = f"NEXUS HOSPITAL INVOICE\nPatient: {p.name}\nTotal Bill: Rs.{p.bill_amt:,}"
    ...
    def do_assign(self):
        for p in self.patients:
            if p.name == self.as_p.get():
                ...
                self.save_data()
                messagebox.showinfo("Success", f"Dr. {self.as_d.get()} assigned.")
                self.show_assign()
                return
    ...
    def do_dis(self):
        for p in self.patients:
            if p.name == self.di_p.get():
                if p.bill_amt == 0:
                    messagebox.showwarning("Warning", "Generate bill first.")
                    return
                p.status = "Discharged"
                self.pay_engine.gross_revenue += p.bill_amt
                self.save_data()
                messagebox.showinfo("Success", f"{p.name} Discharged.")
                self.show_discharge()
                return
